...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['id'])) {
    ...
    if ($result && $db->num_rows($result) > 0) {
        ...
        $checkout_id = (int)$checkout_item['id'];  // Get the checkout_id
        ...
        if ($product_result && $db->num_rows($product_result) > 0) {
            ...
            if ($db->query($update_query)) {
                // Now delete the checked-out entry after check-in
                $delete_checkout_query = "DELETE FROM check_out WHERE item_id = '{$product_id}' AND id = '{$checkout_id}'";
                $db->query($delete_checkout_query); // Execute the delete query
